...
export default function AccountingInvoicesPurchase({ onRefresh, dateParams }) {
  ...
  const load = useCallback(async () => {
    ...
    const [inv, acc] = await Promise.all([
      apiClient(`/api/accounting/invoices?type=purchase${q}`).then(r => r.json()).catch(() => null),
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} style={{ padding: 36, textAlign: 'center', color: C.muted, fontSize: 13 }}>No bills found.</td></tr>
            ) : filtered.map((inv, i) => {
